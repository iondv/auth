...
const { StrategyProvider } = require('@iondv/accounts-contracts');
...
const { log: { LoggerProxy }, utils: { duration: parseDuration } } = require('@iondv/commons');
        ...
        function (err, user) {
          if (err) {
            if (typeof err.failInfo === 'undefined') {
              ...
              req.flash('error', typeof err === 'string' ? err : err.getMessage(req.locals.lang));
            }
            res.redirect(self);
          } else if (user) {
            req.logIn(user, (err) => {
      ...
      validatePassword(req.body, (err) => {
        ...
        validate(req.body)
          ...
          .then(user => _this.register(user, function (err, u) {
            if (err) {
              log.error(err);
              req.flash('error', t('User registration failed!', {lang: req.locals.lang}));
              return res.redirect(self);
            }
    ...
    return function (req, res, next) {
      ...
      validate(req.body)
        .then(output => options.eventLogger.logChange(eventLoggerTypes.EDIT_PROFILE,
          ...
          .then(() => options.accounts.set(req.user.id(), {properties: output})))
        .then((user) => {
          user.setProperties({ip: req.ip});
        ...
        validatePassword(
          ...
          (err) => {
            ...
            options.accounts
              ...
              .catch((err) => {
                req.flash('error', err.getMessage(req.locals.lang));
                return res.redirect(self);
              });
          },
          req.user.id()
